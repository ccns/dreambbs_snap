  ...
  BANMAIL *ban;
{
  ...
  sprintf(modes,"%c%c%c%c%c",(ban->mode&FW_OWNER)?'1':'0',(ban->mode&FW_TITLE)?'1':'0',
          (ban->mode&FW_TIME)?'1':'0',(ban->mode&FW_PATH)?'1':'0',
          (ban->mode&FW_ORIGIN)?'1':'0');
  now = (int)((ban->time - time(0) + BANMAIL_EXPIRE*86400)/3600);
  prints("%6d  %6d %6d %s  %-48.48s\n", num, ban->usage,
  now < 0 ? 0 : now,modes,ban->data);
}
  ...
  XO *xo;
{
  ...
  if (!fstat(fd, &st) && (size = st.st_size) > 0)
  {
    ...
    if (size >= sizeof(BANMAIL))
    {
      ...
      {
        if (size > sizeof(BANMAIL))
          xsort(pbase, size / sizeof(BANMAIL), sizeof(BANMAIL), str_cmp);
  ...
  int echo;
{
  ...
  sprintf(modes,"%c%c%c%c%c",(banmail->mode&FW_OWNER)?'1':'0',(banmail->mode&FW_TITLE)?'1':'0',
  ...
  if(vget(b_lines, 0, "擋信列表：", banmail->data, sizeof(banmail->data), echo))
    change++;
  sprintf(buf,"擋信模式：(作者,標題,時間,路徑,來源)[%s]",modes);
  if(vget(b_lines,0,buf,modes,6,GCARRY))
  {
