  ...
  int paling;			/* 是否正在設定好友名單 */
{
  ...
  prints("上次(\033[1;33m%s\033[m)來自(\033[1;32m%s\033[m)\n", 
  ...
  mail = m_query(userid);
  if(mail)
    prints(" [信箱] \033[1;31m有 %d 封新情書\033[m",mail);
  else
    prints(" [信箱] 都看過了");

  /* deny nick can't have plans. statue.2001.02.11 */
  if(!(acct->userlevel & PERM_DENYNICK) || HAS_PERM(PERM_SYSOP))
    showplans(userid);
  vmsg(NULL);
...
#if 0  /* Thor.990201: 註解: 不用加了, 因為 tv_sec每次都會重設為原值再select */
#ifdef LINUX
...
KeyFunc ulist_cb[] =
{
  ...
  'q', ulist_query,
  'b', ulist_broadcast,
  'z', ulist_init,		/* refresh status Thor: 應user要求 */
  'c', t_cloak,
  'R', ulist_realname,
  'o', ulist_mp,
  'd', ulist_del,
...
#ifdef	LINUX
  /*
   * Linux 下連續 page 對方兩次就可以把對方踢出去： 這是由於某些系統一 signal
   * 進來就會將 signal handler 設定為內定的 handler, 不幸的是 default 是將程式
   * terminate. 解決方法是每次 signal 進來就重設 signal handler
   */
  ...
  char *fpath;
{
  ...
  if (!fstat(fd, &st) && (size = st.st_size) > 0)
  {
	...
	if (size > sizeof(BANMSG))
	{
	  xsort(pbase, size / sizeof(BANMSG), sizeof(BANMSG), str_cmp);
	}
