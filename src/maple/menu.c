...
#ifdef	HAVE_INFO
#define INFO_EMPTY	"Info      【 \033[34m校方公告區\033[m 】"
#define	INFO_HAVE	"Info      【 \033[41;33;1m快進來看看\033[m 】"
#endif

#ifdef	HAVE_STUDENT
#define	STUDENT_EMPTY	"Student   【 \033[34m學生公告區\033[m 】"
#define STUDENT_HAVE	"Student   【 \033[41;33;1m快進來看看\033[m 】"
#endif
...
goodbye()
{
  ...
  if(cuser.ufo2 & UFO2_DEF_LEAVE)
  {
    ...
    ans = vans("G)再別" NICKNAME " M)報告站長 N)留言板 Q)取消？[G] ");
...
/*  int color = (time(0) % 7) + 41;        lkchu.981201: random color */
...
#endif
#if 0
  char *newyear[6]={"風 之 塔 祝 大 家 新 年 快 樂 ",
                    " 2003 年 也 要 支 持 風 之 塔 ",
                    " 大 家 幫 忙 多 拉 點 人 來 喔",
                    " 風 之 塔 是 最 溫 馨 的 地 方",
                    "       風 之 塔 萬 歲!!       ",
                    " 新 年 以 後 大 家 要 努 力 喔"};
  ...
  else
  {
    if (ufo & UFO_BIFF)
    {
      mid = "\033[41;37;5m 你有新情書! \033[0;1;33;44m";
      spc = 15;
    }
    else if (ufo & UFO_BIFFN)
    {
      mid = "\033[42;37;5m 你有新留言! \033[0;1;33;44m";
      spc = 15;
    }
#if 0
    else if(time(0) < 1041436800)
    {
      ...
      else
      {
        ...
        min = now/60 - hour*60;
        sec = now % 60;
        sprintf(ttl,"距 2003 年還剩 %02d 時 %02d 分 %02d 秒",hour,min,sec);
        spc = strlen(ttl);
        mid = ttl;  
        mid[spc] = '\0';
      }
    ...
    {
        sprintf(ttl,"據統計，目前有 %d 個塔民受騙",ushm->avgload);
        spc = strlen(ttl);
        mid = ttl;
        mid[spc] = '\0';
    }
...
#ifdef	HAVE_PROFESS
  Profess, 0, M_PROFESS,
  "Profession【 \033[1;36m專業討論區\033[m 】",
#endif
...
#ifdef HAVE_SIGNED_MAIL
  menu_mail, 0 , M_MAIL, /* Thor.990413: 若不用m_verify, guest就沒有選單內容囉 */
  "Mail      【 信件工具區 】",
#else
  menu_mail, 0 , M_MAIL,
  "Mail      【 私人信件區 】",
