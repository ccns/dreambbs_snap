...
#ifdef	HAVE_PROFESS
static char *profess_img;
#endif

#ifdef	HAVE_FAVORITE
static char *favorite_img;
#endif
  ...
  XO *xo;
{
  ...
  do
  {
	...
	if (brdnew)
	{
	  ...
	  num = brd->bpost;
	  str = brd->blast > brd_visit[chn] ? "\033[1;32m£¾\033[m" : "¡C";
                                              /* lkchu.981201: ¤W¦â¸û²M·¡ :p */                                              
	}
	else
	{
	  num = cnt;
  ...
  XO *xo;
{
  ...
  prints("[¡ö]¥D¿ï³æ [¡÷]¾\\Åª [¡ô¡õ]¿ï¾Ü [c]½g¼Æ [y]¸ü¤J [/]·j´M [s]¬ÝªO [h]»¡©ú\n"
    "[30;47m  %-7s¬Ý  ªO            %-33s§ë²¼ ªO    ¥D     [m",
    class_flag & UFO2_BRDNEW ? "Á`¼Æ" : "½s¸¹", "¤¤   ¤å   ±Ô   ­z");
