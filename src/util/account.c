  ...
  int prune;
{
  ...
  sprintf(buf, "/usr/bin/tar cfz log/%s%s.tgz %s", target, stamp, source);
  system(buf);
...
gem_backup()
{
  ...
  if (!(op & 7))
  {
    system("/usr/bin/tar cfz /usr/data/gem/gem.tgz .??? ?");
  }
  ...
  if (dirp = opendir("."))
  {
	...
	continue;
      ...
      rename(tgz, buf);
      sprintf(buf, "/bin/gtar cfz %s %s", tgz, fname);
      system(buf);
    }
    closedir(dirp);
  }
...
//    keeplog(FN_BANMAIL_LOG, NULL, title, 2);
    ...
    keeplog(FN_POP3_LOG, NULL, title, 2);
...
//    keeplog(FN_INNBBS_LOG, BRD_SECRET, title, 2);
    ...
    keeplog(FN_PIPMONEY_LOG, BRD_SECRET, title, 2);
...
//    keeplog(FN_EXPIRE_LOG, BRD_SECRET, title, 2);
    ...
    sprintf(title, "[記錄] %s看版信件手動刪除紀錄", date);
    ...
    /* 由 reaper 產生 */
    sprintf(title, "[記錄] %s偷懶版主紀錄", date);
    keeplog(FN_LAZYBM_LOG, NULL, title, 2);
/*
    sprintf(title, "[記錄] %s相同的 E-mail 紀錄", date);
