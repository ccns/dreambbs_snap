  ...
  *ptr++ = 0;
  ...
  { /* Thor.990318: 查為什麼寫不進去 */
    ...
    if(ret < 0)
    {
      ...
      fprintf(fp, "REG-\t[%d] <%s>  write fail(%s) %s\n", sno, acct.userid, sys_errlist[errno], justify);
      logitvalid(FN_VERIFY_LOG, "-BMTA Verify ERR-", acct.userid , acct.email);
      return -1;
    }
  }
  ...
  Agent *ap;
{
  ...
  if (cc < 0)
  {
    ...
    if (cc != EWOULDBLOCK)
    {
      agent_log(ap, "SEND", sys_errlist[cc]);
      return 0;
    }
...
#undef	HEADER_SIZE
  ...
  if (cc <= 0)
  {
    ...
    if (cc != EWOULDBLOCK)
    {
      agent_log(ap, "RECV", sys_errlist[cc]);
      return 0;
    }
