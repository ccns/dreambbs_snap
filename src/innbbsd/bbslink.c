...
/* SoC + */
#define LOCALDAEMON (_PATH_BBSHOME "/innd/.innbbsd")
...
{
  char *n;
  char poster[MAXBUFLEN], filepath[MAXPATHLEN];
  soverview_t sover;
  ...
  soverview_t *sover;
{
  ...
  FILE *ECHOMAIL, *FN;
  char *hash;
  char times[MAXBUFLEN], filepath[MAXPATHLEN];
  time_t mtime;
  ...
  else
  {
	...
	*bufferp = '\n';
    ...
    if (bufferp)
    {
      for (buffer = BODY = bufferp + 1, bufferp = strchr(buffer, '\n');
	buffer && *buffer; bufferp = strchr(buffer, '\n'))
      {
	if (bufferp)
	  *bufferp = '\0';
  ...
  char *textline;
{
  ...
  if (NNTPHOST && *NNTPHOST)
    fprintf(NNTPwfp, "%s: %s\r\n",
      USEIHAVE ? "NNTP-Posting-Host" : "X-Auth-From" , NNTPHOST);
  ...
  else if (USEPOST)
  {
    if (status == 240)
    {
      /* Thor.990304: 為了讓 POST 也能不發出fake cancel, 和被外界 cancel */
  ...
  char *hostname, *hostprot, *hostport;
{
  ...
  if (!USEDATA)
  {
	...
	/* exit( 0 ); */
      ...
      /* SoC: open connection to innbbsd */
      {
...
/* let go after Aug. 3 */
      ...
      {
        /* Thor.980529: 看看要發的cmsg所cancel的message, 的id 有沒有在his */
                      /* cmsg的message id 格式是 ####.msgid */
                      /* 在此假設 cmsg的msgid 一定有 "." */
	fprintf(innbbsout, "grephist <%s>\r\n", strchr(soverview.msgid,'.') + 1);
	fflush(innbbsout);
	fgets(INNBBSbuffer, sizeof INNBBSbuffer, innbbsin);
...
send_article()
{
  ...
  for (nlcount = 0; nlcount < NLCOUNT; nlcount++)
  {
	...
	*right = ')';
    ...
    sprintf(feedfile, "innd/%s.feed", site);
    sprintf(feedingfile, "innd/%s.feeding", site);
	...
	if (bbslink_get_lock(feedfile) && bbslink_get_lock(feedingfile))
	{
	  ...
	  system("innd/ctlinnbbsd reload > /dev/null");
	  send_nntplink(node, site, nntphost, protocol, port, feedingfile, nlcount);
  ...
  char **argv;
{
  ...
  if (left && right)
  {
    ...
    *right = ')';
    ...
    if (!rename("innd/out.bntp", OUTING) && bbslink_get_lock(bbslink_lockfile))
    {
      /* When try to visit new post, try to lock it */
      FILE *NEWPOST = fopen(OUTING, "r");
      if (NEWPOST == NULL)
      {
	bbslog("<bbslink> Err: can't open %s\n", OUTING);
	...
	*ptr++ = '\0';
    ...
    if (!rename("innd/cancel.bntp", OUTING) && bbslink_get_lock(bbslink_lockfile))
    {
    FILE *CANCELFILE;

      CANCELFILE = fopen(OUTING, "r");
      while (fgets(result, sizeof result, CANCELFILE))
      {
	/* chop( $_ ); */
	char *board, *filename, *userid, *nickname, *subject;
	char *ptr, **sptr;
...
#define DEBUGBBSLINK
  ...
  while ((c = getopt(argc, argv, "s:hnvVk")) != -1)
    switch (c)
    {
    case 's':
...
/*  bbslog("addhist <%s> %s/%s\r\n", msgid, board, filename); */
  fflush( innbbsout);
  fgets(INNBBSbuffer, sizeof INNBBSbuffer, innbbsin);
/*   bbslog("%s",INNBBSbuffer); */
}
/* SoC - */
