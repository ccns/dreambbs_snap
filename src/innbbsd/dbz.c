/* #define ORIG_DBZ to usr original version */
  ...
  char *name;
{
   ...
   * unbuffered i/o does not cure the problem, but does enormously reduce its
   * incidence.
   */
  ...
  char *s2;
{
  ...
  p = malloc((len = (size_t) strlen(s1)) + (size_t) strlen(s2) + 1);
  if (p != NULL)
  {
    (void) memcpy(p, s1, len);
    (void) strcpy(p + len, s2);
  }
  else
  {
  ...
  datum key;
{
  ...
  while ((key_ptr = search(&srch)) != NOTFOUND)
  {
    ...
    /* fetch the key */
    if (fseek(fp, key_ptr, SEEK_SET) != 0)
    {
      DEBUG(("fetch: seek failed\n"));
      return (output);
    }
    if (fread((POINTER) buffer, 1, keysize, fp) != keysize)
    {
  ...
  register struct dbzconfig *cp;
{
  ...
  (void) fprintf(f, "dbz %d %ld %d %c %ld %ld %d %d", dbzversion, cp->tsize,
    ...
    cp->tagmask, cp->tagshift, value = cp->valuesize);
  for (i = 0, ari = cp->bytemap; i < value; i++)
    (void) fprintf(f, " %d", *ari++);
  (void) fprintf(f, "\n");
  for (i = 0, arl = cp->used; i < NUSEDS; i++)
    (void) fprintf(f, "%ld%c", *arl++, (i < NUSEDS - 1) ? ' ' : '\n');
...
#ifdef MMAP
  register int fd;
  register size_t fsize;
  struct stat st;

  if (fstat(fd = fileno(f), &st) == -1)
  {
    DEBUG(("getcore: fstat failed\n"));
    return (NULL);
  }
  fsize = (size_t) conf.tsize * SOF;
  if (fsize > st.st_size)
  {
    /* file too small; extend it */
    if (ftruncate((int) fd, fsize) == -1)
    {
      DEBUG(("getcore: ftruncate failed\n"));
      return (NULL);
    }
  }
  it = mmap((caddr_t) 0, (size_t) fsize,
  ...
  register struct searcher *osp;/* may be FRESH, i.e. NULL */
{
  ...
  {
    sp->hash = h;
    sp->tag = MKTAG(h / (tsize = conf.tsize));
    DEBUG(("tag 0x%lx\n", sp->tag));
    sp->place = h % tsize;
    sp->tabno = 0;
    sp->run = (conf.olddbz) ? tsize : MAXRUN;
    sp->aborted = 0;
  }
  sp->seen = 0;
}
  ...
  register struct searcher *sp;
{
  ...
  for (;;)
  {
	...
	sp->run = MAXRUN;
      ...
      sp->place = place = (place + 1) % tsize + sp->tabno * tsize;
    }
    else
  ...
  of_t value;
{
  ...
  fp = pagf;
  if (fseek(fp, (of_t) (place * SOF), SEEK_SET) != 0 ||
     (fwrite((POINTER) & value, SOF, 1, fp) != 1) ||
     (fflush(fp) == EOF))
  {
    sp->aborted = 1;
    return (-1);
