...
char *BODY;
char *FROM, *SUBJECT, *SITE, *DATE, *POSTHOST, *NNTPHOST, *PATH, *GROUPS,
    *MSGID, *CONTROL;
...
readlines(client)
  ClientType *client;
{
  register char *front, *ptr, *hptr, *tp;
  register int i;
  /* int fd = client->fd; */
  ...
  for (ptr = (char *) strchr(front, '\n'); ptr && *ptr; front = ptr + 1, ptr = (char *) strchr(front, '\n'))
  {
    *ptr = '\0';
    ...
    if (hptr != NULL && hptr[1] == ' ')
    {
      *hptr = '\0';
      ...
      if (i != -1)
      {
	if (tp = (char *) strchr(HEADER[i] = hptr + 2, '\r'))
	  *tp = '\0';
      }
      *hptr = ':';
    }
  ...
  if (POSTHOST == NULL)
  {
    if (ptr = HEADER[X_Auth_From_H])
      HEADER[NNTPPOSTINGHOST_H] = POSTHOST = ptr;
  }
