...
bw_init()
{
  ...
  sprintf(buf,"☆%s(%d戰%d勝) vs ★%s(%d戰%d勝) \033[m",cuser.userid, myTotal, myWin, cutmp->mateid, yourTotal, yourWin);
  sprintf(noansi,"【 對奕%s 】 %s vs %s",ruleStr,cuser.userid,cutmp->mateid);
  strcpy(whatsay,buf);
...
int BWboard(int sock, int later)
{
  ...
  if(!later)
  {
    ...
    c = vans("想下哪種棋? 0)取消 1)黑白棋 2)五子棋 3)圍棋 [0]");
    if(c) c-='0';
    else 
